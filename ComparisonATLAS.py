...
labels=["NLO"]+["NLO \oplus NLL"]
types=["Diff"]
#types=["Born","Resum"]
color=['b','k','b','magenta','orange','m','maroon','orange']
...
def BuildPlot():
    ...
    axis[0].errorbar(rangeM,AtlasM,yerr=Atlaserr,xerr=errange,fmt=',r',ecolor='r',elinewidth=0.5,capsize=1.5,barsabove=True,label=r'${\rm \mathbf{ATLAS}}$ [183]')
    ...
    axis[0].fill_between(rangeL,Matchedm,Matchedp,alpha=0.5,linewidth=1,antialiased=True,facecolor=color[len(types)+5],label=r"${\rm \mathbf{NLO \oplus  N^2LL'}}$ [182]",step='post')
    ...
    DeltaNLO=np.mean((resMGExtNLO[1]-resMGExtNLO[0])/resMGNLO[0])    
    ...
    axis[1].fill_between(rangeL,Matchedm,Matchedp,alpha=0.5,linewidth=1,antialiased=True,facecolor=color[len(types)+5],label=r'${\rm \mathbf{NLO + NNLLp \, [182]}}$ ',step='post')
    ...
    #handles, labels2 = axis[0].get_legend_handles_labels()
    #axis[0].legend([handles[idx] for idx in order],loc='best', fontsize = 12)
    ...
    axis[1].set_xlim([2*173.2,1600])
    axis[0].set_xlim([173.2*2,1600])
    ...
    axis[1].set_ylabel(r'$\sigma/\sigma_{{\rm res.}}$',fontsize=16)
    axis[1].set_xlabel(r'${\rm M_{t \bar{t}}}$ [GeV]',fontsize=16)
        
    axis[0].set_title(r'$t \bar{t} \; {\rm production \; cross \; section} \sqrt{s}=8 \; {\rm TeV}$', y=1.1, pad=0, x=0.5)
    figure.tight_layout()
    plt.subplots_adjust(hspace=0.0)
    plt.savefig('ComparisonATLAS.svg')
    plt.savefig('/home/yehudi/Documents/Thesis/Manuscript/chapters/images/chapter05/ComparisonATLAS.png',dpi=800)
    #plt.savefig('/home/yehudi/Documents/Thesis/Manuscript/chapters/images/chapter05/ComparisonATLAS.pgf')
    plt.legend([])
    plt.close() 
BuildPlot()
