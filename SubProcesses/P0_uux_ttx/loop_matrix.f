...
C     HSS DEBUG
      ...
      COMMON/ML5_0_DOUBLECHECK_HF/HF_FROM_JAMP
...
C     RETURNCODE=100*RET_CODE_H+10*RET_CODE_T+RET_CODE_U
      ...
      DATA (ACCURACY(I,1),I=1,2)/2*1.0D0/
      DATA (ACCURACY(I,2),I=1,2)/2*1.0D0/
...
C     HSS DEBUG
      DO I=1,NCOLORBASIS
        ...
        ENDDO
      ENDDO
      CALL ML5_0_SMATRIXHEL_SPLITORDERS(P_USER,USERHEL,BORNBUFF)
      DO I=1,NCOLORBASIS
         DO J=1,NCOLORBASIS
...
C            argument of the subroutine below).
            ...
            IF(ME_COMPUTATION_FROM_JAMP) THEN
              CALL ML5_0_COMPUTE_HARDFUNCTION(BUFFRES,HEL_MULT)
              IF(((.NOT.DIRECT_ME_COMPUTATION)
...
C                     not using DABS to keep the possible phase difference
     ...
     $                      +DIMAG(HELSAVED(3,I,J,H))
                    ENDDO
                  ENDDO
                  DUMMY=ML5_0_ISSAME(TMPR0,TMPR1,REF,.FALSE.)
                  IF(DUMMY.NE.0) THEN
                    IF(NTRY.EQ.1) THEN
...
C              not using DABS to keep the possible phase difference
     ...
     $                    ABS(GOODHEL(HELPICKED)+HELOFFSET)))
               ...
               ENDDO
            ENDDO
            IF(ML5_0_ISSAME(TMPR0,TMPR1,REF,.TRUE.).EQ.0) THEN
              WRITE(*,*) '##W15 Helicity filter could not be'
     $         //' successfully double checked.'
              WRITE(*,*) '##One reason for this is that you might have'
...
C     -----------
     ...
     $   //' maximum ',NBORNAMPS
      ENDIF
C     HSS DEBUG
      ML5_0_ML5SOINDEX_FOR_BORN_AMP = 1
...
C     
      ...
      DATA (LOOPAMPORDERS(I),I= 36, 37) /    2,    2/
C     -----------
C     BEGIN CODE
C     -----------
     ...
     $   //' maximum ',NLOOPAMPS
      ENDIF
C     HSS DEBUG
      ML5_0_ML5SOINDEX_FOR_LOOP_AMP = 1
