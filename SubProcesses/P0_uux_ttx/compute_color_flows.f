...
C      AMPL onto the color flow space.
      ...
      ! HSS DEBUG
      CALL ML5_0_DO_COMPUTE_NEW_COLOR_FLOWS(.FALSE.)
      END SUBROUTINE
      ...
      ! HSS DEBUG
      CALL ML5_0_DO_COMPUTE_NEW_COLOR_FLOWS(.TRUE.)
      END SUBROUTINE
...
C       Initialize the BornColorProjector
 ...
 301    CONTINUE
        STOP 'Color projection coefficients could not be initialized'
     $   //' from file ML5_0_BornColorFlowCoefs.dat.'
 303    CONTINUE
        CLOSE(1)
...
C     Start by projecting the Born amplitudes
      DO I=1,NBORNFLOWS
        DO J=1,SIZE(BORNCOLORPROJECTOR(I)%AMPID)
          SOINDEX = ML5_0_ML5SOINDEX_FOR_BORN_AMP(BORNCOLORPROJECTOR(I)
     $     %AMPID(J))
...
C     This is of course not F77 standard but widely supported by now.
     ...
     $  BORNCOLORFLOWMATRIX
      ...
      COMMON/ML5_0_FLOW_COLOR_MATRIX/LOOPCOLORFLOWMATRIX,
...
C              correspond to
              CALL ML5_0_ML5GET_ORDERS_FOR_AMPSOINDEX(M,ORDERS_A)
              CALL ML5_0_ML5GET_ORDERS_FOR_AMPSOINDEX(N,ORDERS_B)
...
C              absence of the ML5 prefix in the functions used then)
              ISQSO =
     $          ML5_0_SQSOINDEX(ML5_0_SOINDEX_FOR_AMPORDERS(ORDERS_A)
     $         ,ML5_0_SOINDEX_FOR_AMPORDERS(ORDERS_B))
              IF(J.NE.I) THEN
                DOUBLEFACT=2
              ELSE
                DOUBLEFACT=1
...
C     ----------
      ...
      SUBROUTINE ML5_0_DO_COMPUTE_NEW_COLOR_FLOWS(DUMMY)
      IMPLICIT NONE
      LOGICAL DUMMY
      INTEGER I,J
      ...
      COMMON/ML5_0_JAMPL_OCB/JAMPL_OCB
      COMPLEX*16 JAMPB_OCB(NCOLORBASIS)
      COMMON/ML5_0_JAMPB_OCB/JAMPB_OCB
     ...
     $           /DBLE(ColorRotateDen(I,J))*JAMPL(3,J,1)
      ...
      SUBROUTINE ML5_0_COMPUTE_HARDFUNCTION(RES,HEL_MULT)
      IMPLICIT NONE
      INCLUDE 'NCOLORBASIS.inc'
