  ...
  for(int k=0; k < Len; k++)
      ...
      for(int l=0; l < Len; l++)
	{
	  for(int mm=0; mm < Len; mm++)
		...
		{
		 Ht[k][l]+=R[k][mm]*(xx[0+nn*4+mm*Len]*pow(alpha_s/alpha_p,2)+xx[1+nn*4+mm*Len]*pow(alpha_s/alpha_p,3))/2./M2*Rdag[nn][l];
  ...
  for(int ka=0; ka < Len; ka++)
     {
       for(int ji=0; ji < Len; ji++)
	 {
	   res+=(S0(ka,ji,chan)+S1t(x,ka,ji,chan,M2)+SG(x,ka,ji,chan,M2))*xx[0+ka*4+ji*Len]*pow(alpha_s/alpha_p,2)/2./M2;
	   ...
	   res+=S0(ka,ji,chan)*xx[1+ka*4+ji*Len]*pow(alpha_s/alpha_p,3)/2./M2; //Virtual contribution
	 }
     }
  //exit(2);
