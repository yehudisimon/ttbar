...
C     HSS DEBUG
C      REAL*8 , ALLOCATABLE :: MATELEM(:,:)
      COMPLEX*16, ALLOCATABLE :: MATELEM(:,:,:)
...
C     HSS DEBUG
C      REAL*8 , ALLOCATABLE :: PREC_FOUND(:)
...
C     
      ...
      COMMON/ML5_2_CHOSEN_BORN_SQSO/CHOSEN_BORN_SO_CONFIGS
...
C        ALLOCATE(MATELEM(0:3,0:MATELEM_ARRAY_DIM))
        ALLOCATE(MATELEM(0:3,NCOLORBASIS,NCOLORBASIS))
        CALL ML5_2_GET_NSQSO_LOOP(NSQUAREDSO_LOOP)
C       HSS DEBUG
C        ALLOCATE(PREC_FOUND(0:NSQUAREDSO_LOOP))
        ALLOCATE(PREC_FOUND(NCOLORBASIS,NCOLORBASIS))
...
C           HSS DEBUG
            IF (PREC_FOUND(1,1).GT.0.0D0) THEN
              WRITE(*,'(1x,a23,1x,1e10.2)') 'Relative accuracy     ='
     $         ,PREC_FOUND(1,1)
            ELSEIF (PREC_FOUND(1,1).EQ.0.0D0) THEN
              WRITE(*,'(1x,a23,1x,1e10.2,1x,a30)') 'Relative accuracy '
     $         //'    =',PREC_FOUND(1,1),
     $              '(i.e. beyond double precision)'
            ELSE
     ...
     $       //' for the loop summed result below:'
          ...
          WRITE (*,*) '---------------------------------'
C         HSS DEBUG
          OPEN(FILE="born_HF.dat",UNIT=2030)
          WRITE (*,*) 'Matrix element born = ', MATELEM(0,1,1)
          DO I=1,NCOLORBASIS
             WRITE(2030,*)(MATELEM(0,I,J),J=1,NCOLORBASIS)
          ENDDO
          CLOSE(UNIT=2030)
C     $     ' GeV^',-(2*NEXTERNAL-8)
          WRITE (*,*) 'Matrix element finite = ', MATELEM(1,1,1), 
     $     ' GeV^',-(2*NEXTERNAL-8)
C         HSS DEBUG
          OPEN(FILE="finite_HF.dat",UNIT=2030)
          DO I=1,NCOLORBASIS
             WRITE(2030,*)(MATELEM(1,I,J),J=1,NCOLORBASIS)
          ENDDO
          CLOSE(UNIT=2030)
          WRITE (*,*) 'Matrix element 1eps   = ', MATELEM(2,1,1), 
     $     ' GeV^',-(2*NEXTERNAL-8)
C         HSS DEBUG
          OPEN(FILE="singlepole_HF.dat",UNIT=2030)
          DO I=1,NCOLORBASIS
             WRITE(2030,*)(MATELEM(2,I,J),J=1,NCOLORBASIS)
          ENDDO
          CLOSE(UNIT=2030)
          WRITE (*,*) 'Matrix element 2eps   = ', MATELEM(3,1,1), 
     $     ' GeV^',-(2*NEXTERNAL-8)
C         HSS DEBUG
          OPEN(FILE="doublepole_HF.dat",UNIT=2030)
          DO I=1,NCOLORBASIS
             WRITE(2030,*)(MATELEM(3,I,J),J=1,NCOLORBASIS)
          ENDDO
          CLOSE(UNIT=2030)
          WRITE (*,*) '---------------------------------'
          IF (DREAL(MATELEM(0,1,1)).NE.0.0D0) THEN
            WRITE (*,*) 'finite / (born*ao2pi) = ', MATELEM(1,1,1)
     $       /MATELEM(0,1,1)/AO2PI
            WRITE (*,*) '1eps   / (born*ao2pi) = ', MATELEM(2,1,1)
     $       /MATELEM(0,1,1)/AO2PI
            WRITE (*,*) '2eps   / (born*ao2pi) = ', MATELEM(3,1,1)
     $       /MATELEM(0,1,1)/AO2PI
          ELSE
            WRITE (*,*) 'finite / ao2pi      = ', MATELEM(1,1,1)/AO2PI
            WRITE (*,*) '1eps   / ao2pi      = ', MATELEM(2,1,1)/AO2PI
            WRITE (*,*) '2eps   / ao2pi      = ', MATELEM(3,1,1)/AO2PI
          ENDIF
          WRITE (*,*) '---------------------------------'
...
C         HSS DEBUG
          WRITE (69,'(a4,1x,1ES30.15E3)') 'BORN',DREAL(MATELEM(0,1,1))
          IF (DREAL(MATELEM(0,1,1)).NE.0.0D0) THEN
            WRITE (69,'(a3,1x,1ES30.15E3)') 'FIN',DREAL(MATELEM(1,1,1)
     $       /MATELEM(0,1,1)/AO2PI)
            WRITE (69,'(a4,1x,1ES30.15E3)') '1EPS',DREAL(MATELEM(2,1,1)
     $       /MATELEM(0,1,1)/AO2PI)
            WRITE (69,'(a4,1x,1ES30.15E3)') '2EPS',DREAL(MATELEM(3,1,1)
     $       /MATELEM(0,1,1)/AO2PI)
          ELSE
C           HSS DEBUG
            WRITE (69,'(a3,1x,1ES30.15E3)') 'FIN',DREAL(MATELEM(1,1,1)
...
C         HSS DEBUG
          WRITE (69,'(a3,1x,1e10.4)') 'ACC',PREC_FOUND(1,1)
...
C         HSS DEBUG
          WRITE (69,*) 'SO_Born BORN ',MATELEM(0,1,1)
          WRITE (69,*) 'Split_Orders_Names QCD'
          WRITE (69,*) 'Loop_SO_Results 6'
          WRITE (69,*) 'SO_Loop ACC  ',PREC_FOUND(1,1)
          WRITE (69,*) 'SO_Loop FIN  ',MATELEM(1,1,1)
          WRITE (69,*) 'SO_Loop 1EPS ',MATELEM(2,1,1)
          WRITE (69,*) 'SO_Loop 2EPS ',MATELEM(3,1,1)
          CLOSE(69)
        ELSE
