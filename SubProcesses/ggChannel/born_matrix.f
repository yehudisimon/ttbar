...
C     
      ...
      ! HSS DEBUG
      COMPLEX*16 ANS_SUMMED(NCOLORBASIS,NCOLORBASIS)
C     
C     VARIABLES
C     
      INTEGER I
      ! HSS DEBUG
      INTEGER J
      COMPLEX*16 ANS(NCOLORBASIS,NCOLORBASIS)
C     
C     BEGIN CODE
C     
...
C     
      INTEGER NTRY
      ! HSS DEBUG
      COMPLEX*16 T(NCOLORBASIS,NCOLORBASIS)
      COMPLEX*16 BUFF
      INTEGER K
...
C     
      ...
      COMMON/ML5_2_PROCESS_NHEL/NHEL
...
C      only three external particles.
     ...
     $                 -1.OR.ML5_2_IS_BORN_HEL_SELECTED(IHEL)) THEN
                     ANS(I,J)=ANS(I,J)+T(I,J)
                  ENDIF
                  BUFF=BUFF+T(I,J)
               ENDDO
            ENDDO
            IF (ABS(BUFF).NE.0D0.AND..NOT.GOODHEL(IHEL)) THEN
              GOODHEL(IHEL)=.TRUE.
            ENDIF
          ENDIF
        ENDIF
      ...
      DO I=1,NCOLORBASIS
         DO J=1,NCOLORBASIS
            ANS(I,J)=ANS(I,J)/DBLE(IDEN)
         ENDDO
      ENDDO
      IF(USERHEL.NE.-1) THEN
        DO I=1,NCOLORBASIS
           DO J=1,NCOLORBASIS
              ANS(I,J)=ANS(I,J)*HELAVGFACTOR
           ENDDO
        ENDDO
      ELSE
        DO J=1,NINITIAL
          IF (POLARIZATIONS(J,0).NE.-1) THEN
             DO I=1,NCOLORBASIS
              DO K=1,NCOLORBASIS
                 ANS(I,K)=ANS(I,K)*BEAMS_HELAVGFACTOR(J)
                 ANS(I,K)=ANS(I,K)/POLARIZATIONS(J,0)
              ENDDO
             ENDDO
          ENDIF
        ENDDO
...
C     
      ...
      INCLUDE 'NCOLORBASIS.inc'
      COMPLEX*16 RES(NCOLORBASIS,NCOLORBASIS)
      COMPLEX*16 JAMP_OCB(NCOLORBASIS)
...
C     HSS DEBUG
C     (<ci|A><A|cj>)/(<ci|ci>*<cj|cj>)
      DO I=1,NCOLORBASIS
         DO J=1,NCOLORBASIS
            ! 1/<ci|ci>/<cj|cj>
            TEMP = (DBLE(CiCiDen(I))*DBLE(CiCiDen(J)))
     $           /(DBLE(CiCiNum(I))*DBLE(CiCiNum(J)))
            RES(I,J)=DCMPLX(TEMP,ZERO)*JAMP_OCB(I)*DCONJG(JAMP_OCB(J))
         ENDDO
      ENDDO

      END

      SUBROUTINE ML5_2_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      SUBROUTINE ML5_2_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
...
C     
      ...
      COMMON/ML5_2_PROCESS_NHEL/HELC
...
C     
     ...
     $ //' ML5_2_GET_SQUARED_ORDERS_FOR_SOINDEX'
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' ML5_2_GET_ORDERS_FOR_AMPSOINDEX'
