...
C     
      ...
      COMPLEX*16 ANS(0:3,NCOLORBASIS,NCOLORBASIS)
C     
C     LOCAL VARIABLES 
C     
...
C      I+20.
      ...
      REAL*8 ACC(NCOLORBASIS,NCOLORBASIS)
      COMPLEX*16 DP_RES(3,NCOLORBASIS,NCOLORBASIS,MAXSTABILITYLENGTH)
C     QP_RES STORES THE QUADRUPLE PRECISION RESULT OBTAINED FROM
C      DIFFERENT EVALUATION METHODS IN ORDER TO ASSESS STABILITY.
      COMPLEX*16 QP_RES(3,NCOLORBASIS,NCOLORBASIS,MAXSTABILITYLENGTH)
      INTEGER NHEL(NEXTERNAL), IC(NEXTERNAL)
      INTEGER NATTEMPTS
      DATA NATTEMPTS/0/
      DATA IC/NEXTERNAL*1/
      COMPLEX*16 HELSAVED(3,NCOLORBASIS,NCOLORBASIS,NCOMB)
      INTEGER ITEMP
      LOGICAL LTEMP
      COMPLEX*16 BORNBUFF(NCOLORBASIS,NCOLORBASIS)
      REAL*8 TMPR,TMPR2,TMPR0(3),TMPR1(3)
      COMPLEX*16 BUFFR(3,NCOLORBASIS,NCOLORBASIS)
      COMPLEX*16 BUFFR_BIS(3,NCOLORBASIS,NCOLORBASIS)
      COMPLEX*16 TEMP(0:3,NCOLORBASIS,NCOLORBASIS)
      REAL*8 TEMP1(NCOLORBASIS,NCOLORBASIS)
      COMPLEX*16 CTEMP
      REAL*8 TEMP2(3)
      COMPLEX*16 BUFFRES(0:3,NCOLORBASIS,NCOLORBASIS)
...
C     
      ...
      COMMON/ML5_2_DOUBLECHECK_JAMP/RES_FROM_JAMP
     $ ,DIRECT_ME_COMPUTATION,ME_COMPUTATION_FROM_JAMP


C     HSS DEBUG
      COMPLEX*16 HF_FROM_JAMP(0:3,NCOLORBASIS,NCOLORBASIS)
      COMMON/ML5_2_DOUBLECHECK_HF/HF_FROM_JAMP
...
C      coefficients when changing the CTMode for the stability test.
      ...
      COMMON/ML5_2_SKIP_COEFS/SKIP_LOOPNUM_COEFS_CONSTRUCTION

      LOGICAL TIR_DONE(NLOOPGROUPS)
      COMMON/ML5_2_TIRCACHING/TIR_DONE
...
C     RETURNCODE=100*RET_CODE_H+10*RET_CODE_T+RET_CODE_U
      ...
      REAL*8 ACCURACY(NCOLORBASIS,NCOLORBASIS)
      DATA (ACCURACY(I,1),I=1,3)/3*1.0D0/
      DATA (ACCURACY(I,2),I=1,3)/3*1.0D0/
      DATA (ACCURACY(I,3),I=1,3)/3*1.0D0/
...
C      reading the parameters
     ...
     $ ,FORCED_CHOICE_OF_COLLIER_IR_POLE_COMPUTATION/.FALSE.,.FALSE./
      COMMON/ML5_2_COLLIERPOLESFORCEDCHOICE
...
C     HSS DEBUG
      DO I=1,NCOLORBASIS
        DO J=1,NCOLORBASIS
           BORNBUFF(I,J)=DCMPLX(0.0D0,0.0D0)
        ENDDO
      ENDDO
      CALL ML5_2_SMATRIXHEL_SPLITORDERS(P_USER,USERHEL,BORNBUFF)
      DO I=1,NCOLORBASIS
         DO J=1,NCOLORBASIS
            ANS(0,I,J)=BORNBUFF(I,J)
         ENDDO
      ENDDO
...
C      orders
      REF=0.0D0
C     HSS DEBUG
C     instead, we take the reference as the Born summed over the diagonal orders
      DO I=1,NCOLORBASIS
        REF=REF+DREAL(ANS(0,I,I))
      ENDDO
...
C     HSS DEBUG
            DO I=1,NCOLORBASIS
               DO J=1,NCOLORBASIS
                  ANS(1,I,J)=DCMPLX(0D0,0D0)
                  ANS(2,I,J)=DCMPLX(0D0,0D0)
                  ANS(3,I,J)=DCMPLX(0D0,0D0)
               ENDDO
            ENDDO
            GOTO 9999
          ENDIF
...
C     This code is therefore placed before 205 and after 200.
      CALL ML5_2_REINITIALIZE_CUMULATIVE_ARRAYS()
...
C     HSS DEBUG
        DO I=1,NCOLORBASIS
           DO J=1,NCOLORBASIS
              DO K=0,3
                 HF_FROM_JAMP(K,I,J)=DCMPLX(0D0,0D0)
              ENDDO
           ENDDO
        ENDDO
      ENDIF
...
C      precision or when TIR_CACHE_SIZE<2.
     ...
     $ .NE.5).AND.(ML5_2_TIRCACHE_INDEX(CTMODE).EQ.(TIR_CACHE_SIZE+1)))
     $  THEN
...
C     HSS DEBUG
          DO I=1,NCOLORBASIS
            DO J=1,NCOLORBASIS
              ANS(0,I,J)=HF_FROM_JAMP(0,I,J)
              DO K=1,3
                 ANS(K,I,J)=BUFFR_BIS(K,I,J)
              ENDDO
            ENDDO
          ENDDO
        ENDIF
...
C         Handle the possible requirement of specific polarizations
     ...
     $     .EQ.0.AND.(.NOT.ML5_2_IS_HEL_SELECTED(H))) THEN
            CYCLE
          ENDIF
...
C           HSS DEBUG
            WRITE(*,*)"Error: we should not reach here #2 !"
            STOP
            DO I=1,NLOOPAMPS
              DO J=1,NBORNAMPS
                ...
                ITEMP =
     $            ML5_2_ML5SQSOINDEX(ML5_2_ML5SOINDEX_FOR_LOOP_AMP(I)
     $           ,ML5_2_ML5SOINDEX_FOR_BORN_AMP(J))
                DO K=1,3
                  TEMP2(K) = 2.0D0*HEL_MULT*DBLE(CFTOT*AMPL(K,I)
     $             *DCONJG(AMP(J)))
                ENDDO
...
C          if the computation is only done from the color flows
     ...
     $     .EQ.-1.OR.ML5_2_IS_HEL_SELECTED(H)))) THEN
...
C            argument of the subroutine below).
            CALL ML5_2_COMPUTE_COLOR_FLOWS(HEL_MULT)
            IF(ME_COMPUTATION_FROM_JAMP) THEN
              CALL ML5_2_COMPUTE_HARDFUNCTION(BUFFRES,HEL_MULT)
              IF(((.NOT.DIRECT_ME_COMPUTATION)
...
C                form of computation, we directly update the answer.
                DO K=0,3
                  DO I=1,NCOLORBASIS
                    DO J=1,NCOLORBASIS
                       ANS(K,I,J)=ANS(K,I,J)+BUFFRES(K,I,J)
                    ENDDO
                  ENDDO
                ENDDO
...
C                out this helicity
     ...
     $           -1.OR.ML5_2_IS_HEL_SELECTED(H)) THEN
...
C               HSS DEBUG
                    DO I=1,NCOLORBASIS
                      DO J=1,NCOLORBASIS
                       HF_FROM_JAMP(K,I,J)=HF_FROM_JAMP(K,I,J)
     $                        +BUFFRES(K,I,J)
                      ENDDO
                    ENDDO
                  ENDDO
...
C                out this helicity
     ...
     $           -1.OR.ML5_2_IS_HEL_SELECTED(H)) THEN
                  CALL
     $              ML5_2_COMPUTE_COLOR_FLOWS_DERIVED_QUANTITIES(HEL_MU
     $LT)
                ENDIF
              ENDIF
            ENDIF
...
C      subroutine ML5_2_MP_COMPUTE_LOOP_COEFS to compute OpenLoop
...
C        flows
     ...
     $_DONE)) THEN
      ...
      DO I=1,NLOOPGROUPS
        LTEMP=.TRUE.
C       HSS DEBUG
        IF (.NOT.FILTER_SO.OR.SQSO_TARGET.EQ.1) THEN
          IF (.NOT.S(1,I)) LTEMP=.FALSE.
          DO J=1,3
            ANS(J,1,1)=ANS(J,1,1)+LOOPRES(J,1,I)
          ENDDO
        ENDIF
...
C     HSS DEBUG
      DO I=1,NCOLORBASIS
        DO K=1,NCOLORBASIS
          DO J=1,3
            IF (.NOT.(ANS(J,K,I).EQ.ANS(J,K,I))) THEN
...
C          this helicity
          IF (POLARIZATIONS(0,0).EQ.
     $     -1.OR.ML5_2_IS_HEL_SELECTED(HELPICKED)) THEN
...
C      HSS DEBUG
            DO I=1,NCOLORBASIS
             DO J=1,NCOLORBASIS
               DO K=1,3
                 BUFFR(K,I,J)=BUFFR(K,I,J)+ANS(K,I,J)
               ENDDO
             ENDDO
            ENDDO
          ENDIF
...
C       HSS DEBUG
        HELSAVED(1,1:NCOLORBASIS,1:NCOLORBASIS,HELPICKED)=(0D0,0D0)
        HELSAVED(2,1:NCOLORBASIS,1:NCOLORBASIS,HELPICKED)=(0D0,0D0)
        HELSAVED(3,1:NCOLORBASIS,1:NCOLORBASIS,HELPICKED)=(0D0,0D0)
        DO I=1,NCOLORBASIS
          DO J=1,NCOLORBASIS
            IF (CHOSEN_SO_CONFIGS(1)) THEN
              HELSAVED(1,I,J,HELPICKED)=HELSAVED(1,I,J,HELPICKED)
     $              +ANS(1,I,J)
              HELSAVED(2,I,J,HELPICKED)=HELSAVED(2,I,J,HELPICKED)
     $             +ANS(2,I,J)
              HELSAVED(3,I,J,HELPICKED)=HELSAVED(3,I,J,HELPICKED)
     $             +ANS(3,I,J)
            ENDIF
          ENDDO
        ENDDO
...
C         HSS DEBUG            TMPR=0D0
            DO I=1,NCOLORBASIS
              DO J=1,NCOLORBASIS
                DO K=1,3
                  TMPR=TMPR+ABS(HELSAVED(K,I,J,HELPICKED))
                ENDDO
              ENDDO
            ENDDO
            IF(ML5_2_ISZERO(TMPR
...
C         HSS DEBUG
              ...
              DO I=1,NCOLORBASIS
                DO J=1,NCOLORBASIS
C                  not using DABS to keep the possible phase difference
                   TMPR0(1)=TMPR0(1)+DREAL(HELSAVED(1,I,J,HELPICKED))
     $                  +DIMAG(HELSAVED(1,I,J,HELPICKED))
                   TMPR0(2)=TMPR0(2)+DREAL(HELSAVED(2,I,J,HELPICKED))
     $                  +DIMAG(HELSAVED(2,I,J,HELPICKED))
                   TMPR0(3)=TMPR0(3)+DREAL(HELSAVED(3,I,J,HELPICKED))
     $                  +DIMAG(HELSAVED(3,I,J,HELPICKED))
                ENDDO
              ENDDO
              DO H=1,HELPICKED-1
...
C                 Be looser for helicity check, bring a factor 100
                  ...
                  DO I=1,NCOLORBASIS
                    DO J=1,NCOLORBASIS
C                     not using DABS to keep the possible phase difference
                       TMPR1(1)=TMPR1(1)+DREAL(HELSAVED(1,I,J,H))
     $                      +DIMAG(HELSAVED(1,I,J,H))
                       TMPR1(2)=TMPR1(2)+DREAL(HELSAVED(2,I,J,H))
     $                      +DIMAG(HELSAVED(2,I,J,H))
                       TMPR1(3)=TMPR1(3)+DREAL(HELSAVED(3,I,J,H))
     $                      +DIMAG(HELSAVED(3,I,J,H))
                    ENDDO
                  ENDDO
                  DUMMY=ML5_2_ISSAME(TMPR0,TMPR1,REF,.FALSE.)
                  IF(DUMMY.NE.0) THEN
                    IF(NTRY.EQ.1) THEN
...
C              not using DABS to keep the possible phase difference
                  DO K=1,3
                     TMPR0(K)=TMPR0(K)+DREAL(HELSAVED(K,I,J,HELPICKED))
     $                    +DIMAG(HELSAVED(K,I,J,HELPICKED))
                     TMPR1(K)=TMPR1(K)+DREAL(HELSAVED(K,I,J,
     $                    ABS(GOODHEL(HELPICKED)+HELOFFSET)))
     $                    +DIMAG(HELSAVED(K,I,J,
     $                    ABS(GOODHEL(HELPICKED)+HELOFFSET)))
                  ENDDO
               ENDDO
            ENDDO
            IF(ML5_2_ISSAME(TMPR0,TMPR1,REF,.TRUE.).EQ.0) THEN
              WRITE(*,*) '##W15 Helicity filter could not be'
     $         //' successfully double checked.'
              WRITE(*,*) '##One reason for this is that you might have'
...
C       HSS DEBUG
        DO I=1,NCOLORBASIS
          DO L=1,NCOLORBASIS
            ! we have multiplied the initial average and final symmetry factor
            ANS(K,I,L)=ANS(K,I,L)/DBLE(IDEN)
            IF (USERHEL.NE.-1) THEN
              ANS(K,I,L)=ANS(K,I,L)*HELAVGFACTOR
            ELSE
              DO J=1,NINITIAL
                IF (POLARIZATIONS(J,0).NE.-1) THEN
                  ANS(K,I,L)=ANS(K,I,L)*BEAMS_HELAVGFACTOR(J)
                  ANS(K,I,L)=ANS(K,I,L)/POLARIZATIONS(J,0)
                ENDIF
              ENDDO
            ENDIF
          ENDDO
...
C         HSS DEBUG
          DO I=1,NCOLORBASIS
            DO J=1,NCOLORBASIS
              DO K=1,3
                QP_RES(K,I,J,STAB_INDEX)=ANS(K,I,J)
              ENDDO
            ENDDO
          ENDDO
        ELSE
C         HSS DEBUG
          DO I=1,NCOLORBASIS
            DO J=1,NCOLORBASIS
              DO K=1,3
                DP_RES(K,I,J,STAB_INDEX)=ANS(K,I,J)
              ENDDO
            ENDDO
          ENDDO
        ENDIF
...
C         HSS DEBUG
     ...
     $             //' encountered.'
                ENDIF
C               HSS DEBUG
                WRITE(*,*) '##Accuracies for hard function'
                DO J=1,NCOLORBASIS
                   WRITE(*,*) '##DP accuracies ',J,":"
     $                  //' ',(TEMP1(J,I),I=1,NCOLORBASIS)
                   WRITE(*,*) '##QP accuracies ',J,":"
     $                  //' ',(ACC(J,I),I=1,NCOLORBASIS)
                ENDDO
                WRITE(*,*) 'Best estimate 2eps:'
                DO I=1,NCOLORBASIS
                   WRITE(*,*)(ANS(3,I,J),J=1,NCOLORBASIS)
                ENDDO
                WRITE(*,*) 'Best estimate 1eps:'
                DO I=1,NCOLORBASIS
                   WRITE(*,*)(ANS(2,I,J),J=1,NCOLORBASIS)
                ENDDO
                WRITE(*,*) 'Best estimate fin:'
                DO I=1,NCOLORBASIS
                   WRITE(*,*)(ANS(1,I,J),J=1,NCOLORBASIS)
                ENDDO
                WRITE(*,*) 'Finite double precision evaluations :'
                DO J=1,NCOLORBASIS
                   DO K=1,NCOLORBASIS
                      WRITE(*,*)(DP_RES(1,J,K,I),I=1,N_DP_EVAL)
                   ENDDO
                ENDDO
                WRITE(*,*) 'Finite quad   precision evaluations :'
                DO J=1,NCOLORBASIS
                   DO K=1,NCOLORBASIS
                      WRITE(*,*)(QP_RES(1,J,K,I),I=1,N_QP_EVAL)
                   ENDDO
                ENDDO
...
C     HSS DEBUG
      INCLUDE 'NCOLORBASIS.inc'
      COMPLEX*16 FULLLIST(3,NCOLORBASIS,NCOLORBASIS,MAXSTABILITYLENGTH)
      INTEGER L
      INTEGER LENGTH
      REAL*8 ACC(NCOLORBASIS,NCOLORBASIS)
      COMPLEX*16 ESTIMATE(0:3,NCOLORBASIS,NCOLORBASIS)
C     
C     LOCAL VARIABLES 
C     
...
C       HSS DEBUG
        DO J=1,NCOLORBASIS
          DO L=1,NCOLORBASIS
            DO K=1,3
              FULLLIST(K,J,L,I)=DCMPLX(0D0,0D0)
            ENDDO
          ENDDO
        ENDDO
      ENDDO
        ...
        DO L=1,NCOLORBASIS
          DO I=1,3
            DO J=1,MAXSTABILITYLENGTH
              LIST(J)=ABS(FULLLIST(I,K,L,J))
            ENDDO
            DIFF=MAXVAL(LIST,1,MASK)-MINVAL(LIST,1,MASK)
            AVG=(MAXVAL(LIST,1,MASK)+MINVAL(LIST,1,MASK))/2.0D0
C           always take the first element
C            ESTIMATE(I,K,L)=AVG
            ESTIMATE(I,K,L)=FULLLIST(I,K,L,1)
...
C       The following is used instead
          ...
          AVG = 0.0D0
          DO I=1,3
            ACC(K,L) = ACC(K,L) + ACCURACIES(I)*ABS(ESTIMATE(I,K,L))
            AVG = AVG + ABS(ESTIMATE(I,K,L))
          ENDDO
          IF (AVG.NE.0.0D0) THEN
            ACC(K,L)  = ACC(K,L) / ( ABS(AVG) / 3.0D0)
          ENDIF
...
C        not included
          IF (.NOT.CHOSEN_SO_CONFIGS(1)) THEN
             ACC(1:NCOLORBASIS,1:NCOLORBASIS) = 0.0D0
          ENDIF
...
C        accuracy to 1.0d99.
           DO I=1,3
             DO J=1,MAXSTABILITYLENGTH
               IF (ISNAN(DREAL(FULLLIST(I,K,L,J)))) THEN
                 ACC(K,L) = 1.0D99
               ENDIF
             ENDDO
           ENDDO
...
C     
 ...
 1009   CONTINUE
     ...
     $ //' ML5_2_ML5SOINDEX_FOR_SQUARED_ORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP

      END

      INTEGER FUNCTION ML5_2_GETORDPOWFROMINDEX_ML5(IORDER, INDX)
C     
...
C     
      ...
      ML5_2_GETORDPOWFROMINDEX_ML5=SQPLITORDERS(INDX, IORDER)
...
C     -----------
     ...
     $   //' maximum ',NBORNAMPS
      ENDIF
C     HSS DEBUG
      ML5_2_ML5SOINDEX_FOR_BORN_AMP = 1
...
C     -----------
     ...
     $   //' maximum ',NLOOPAMPS
      ENDIF
C     HSS DEBUG
      ML5_2_ML5SOINDEX_FOR_LOOP_AMP = 1
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' ML5_2_ML5GET_SQUARED_ORDERS_FOR_SOINDEX'
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' ML5_2_ML5GET_ORDERS_FOR_AMPSOINDEX'
...
C     
 ...
 1009   CONTINUE
      ...
      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' ML5_2_ML5SOINDEX_FOR_AMPORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
...
C     
      ...
      REAL*8 PREC_ASKED
      REAL*8 PREC_FOUND(NCOLORBASIS,NCOLORBASIS)
      INTEGER J
C     
C     LOCAL VARIABLES
C     
...
C     ----------
      ...
      IF(ALWAYS_TEST_STABILITY.AND.(H.EQ.1.OR.ACCURACY(1,1).LT.0.0D0))
     $  THEN
        BYPASS_CHECK = .TRUE.
        CALL ML5_2_SLOOPMATRIXHEL(P,HEL,ANS)
        BYPASS_CHECK = .FALSE.
...
C     arrays
      ...
      CALL ML5_2_DEALLOCATE_COLOR_FLOWS()
      CONTINUE
      END


      SUBROUTINE UPDATE_MUR(MUN)
      IMPLICIT NONE
