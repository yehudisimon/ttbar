...
C     HSS DEBUG
      COMPLEX*16 ANSDP(3,NCOLORBASIS,NCOLORBASIS)
C     
C     LOCAL VARIABLES 
C     
      LOGICAL DPW_COPIED
      ...
      DATA IC/NEXTERNAL*1/
C     HSS DEBUG
      COMPLEX*32 ANS(3,NCOLORBASIS,NCOLORBASIS)
C     HSS DEBUG
...
C       This, for now, is only defined for 4-fermionic currents.
      ...
      DO K=1,3
         ...
         DO I=1,NCOLORBASIS
            DO J=1,NCOLORBASIS
               ANSDP(K,I,J)=DCMPLX(0D0,0D0)
               ANS(K,I,J)=CMPLX(ZERO,ZERO,KIND=16)
            ENDDO
         ENDDO
      ENDDO
...
C            containers
 ...
 4000     CONTINUE
          ...
          IF (COMPUTE_INTEGRAND_IN_QP) THEN
C           Copy the multiple precision Born amplitudes computed to
C            the AMP double precision array for its use later if
...
C         HSS DEBUG
          WRITE(*,*)"Error: We cannot reach here #1 !"
          STOP
            DO I=1,NLOOPAMPS
              DO J=1,NBORNAMPS
     ...
     $             ,ML5_2_ML5SOINDEX_FOR_BORN_AMP(J))
                  DO K=1,3
                    TEMP2(K) = 2.0E0_16*HEL_MULT*REAL(CFTOT*AMPL(K,I)
     $               *CONJG(AMP(J)),KIND=16)
                  ENDDO
     ...
     $             ,ML5_2_ML5SOINDEX_FOR_BORN_AMP(J))
                  DO K=1,3
                    DP_TEMP2(K) = 2.0D0*HEL_MULT*REAL(DP_CFTOT
     $               *DP_AMPL(K,I)*CONJG(DP_AMP(J)),KIND=8)
                  ENDDO
...
C         HSS DEBUG
                  DO I=1,NCOLORBASIS
                   DO J=1,NCOLORBASIS
                       IF (COMPUTE_INTEGRAND_IN_QP)THEN
                         ANS(K,I,J)=ANS(K,I,J)+
     $                         CMPLX(BUFFRES(K,I,J),KIND=16)
                       ELSE
                         ANSDP(K,I,J)=ANSDP(K,I,J)+BUFFRES(K,I,J)
                       ENDIF
                   ENDDO
                  ENDDO
                ENDDO
...
C               HSS DEBUG
                DO I=1,NCOLORBASIS
                  DO J=1,NCOLORBASIS
                    HF_FROM_JAMP(0,I,J)=HF_FROM_JAMP(0,I,J)
     $                    +BUFFRES(0,I,J)
                  ENDDO
                ENDDO
...
C     HSS DEBUG
           DO J=1,NCOLORBASIS
              DO K=1,NCOLORBASIS
                 ANSDP(I,J,K)=CMPLX(ANS(I,J,K),KIND=8)
              ENDDO
           ENDDO
        ENDDO
      ENDIF
