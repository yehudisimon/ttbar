...
labels=["LO"]+["NLL_2","NLL_1"]+["LO \, MG","NLO \; MG"]+["NLO \oplus NLL_0"]+["NLO \oplus NLL_2"]+["NLO \oplus NLL_1"]
types=["Born","Diff", "Expand","Resum"]
#types=["Born","Resum"]
color=['b','g','k','r','orange','limegreen','lightcoral','m','c']
...
def BuildPlot():
    ...
    axis[0].step(a,resMG, c=color[len(types)-1], linewidth=0.5,where='post')
    axis[0].fill_between(a,resMGExt[0],resMGExt[1],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[len(types)-1],label=r'${\rm \mathbf{'+labels[len(types)-1]+'}}$ ',step='post')
    axis[0].step(a,resMGExt[0], c=color[len(types)-1], linewidth=0.5, alpha=0.3,where='post')
    axis[0].step(a,resMGExt[1], c=color[len(types)-1], linewidth=0.5, alpha=0.3,where='post')

    axis[0].step(aNLO,resMGNLO, c=color[len(types)], linewidth=0.5,where='post')
    axis[0].fill_between(aNLO,resMGExtNLO[0],resMGExtNLO[1],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[len(types)],label=r'${\rm \mathbf{'+labels[len(types)]+'}}$ ',step='post')
    axis[0].step(aNLO,resMGExtNLO[0], c=color[len(types)], linewidth=0.5, alpha=0.3,where='post')
    axis[0].step(aNLO,resMGExtNLO[1], c=color[len(types)], linewidth=0.5, alpha=0.3,where='post')
    ...
    print("NLL_0/NLO = ",np.mean(combine[0]/resNLO))
    
    axis[0].step(ML,combine[0], c=color[-3], linewidth=0.5,where='post')
    axis[0].fill_between(ML,combineExt[0],combineExt[1],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[-3],label=r'${\rm \mathbf{'+labels[-3]+'}}$ ',step='post')
    axis[0].step(ML,combineExt[0], c=color[-3], linewidth=0.5, alpha=0.3,where='post')
    axis[0].step(ML,combineExt[1], c=color[-3], linewidth=0.5, alpha=0.3,where='post')

    axis[0].step(MR[-2:],combine[0][-2:], c=color[-3], linewidth=0.5,where='pre')
    axis[0].fill_between(MR[-2:],combineExt[0][-2:],combineExt[1][-2:],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[-3],step='pre')
    axis[0].step(MR[-2:],combineExt[0][-2:], c=color[-3], linewidth=0.5, alpha=0.3,where='pre')
    axis[0].step(MR[-2:],combineExt[1][-2:], c=color[-3], linewidth=0.5, alpha=0.3,where='pre')
    ...
    axis[1].step(ML,combine[0],label=r''+labels[-3]+' ', c=color[-3], linewidth=0.5,where='post')
    axis[1].step(ML,combineExt[0], c=color[-3], linewidth=0.5, alpha=0.3,where='post')
    axis[1].step(ML,combineExt[1], c=color[-3], linewidth=0.5, alpha=0.3,where='post')
    axis[1].fill_between(ML,combineExt[0],combineExt[1],alpha=0.5,facecolor=color[-3],linewidth=1,antialiased=True,step='post')

    axis[1].step(MR[-2:],combine[0][-2:], c=color[-3], linewidth=0.5,where='pre')
    axis[1].step(MR[-2:],combineExt[0][-2:], c=color[-3], linewidth=0.5, alpha=0.3,where='pre')
    axis[1].step(MR[-2:],combineExt[1][-2:], c=color[-3], linewidth=0.5, alpha=0.3,where='pre')
    axis[1].fill_between(MR[-2:],combineExt[0][-2:],combineExt[1][-2:],alpha=0.5,facecolor=color[-3],linewidth=1,antialiased=True,step='pre')

        
    axis[1].step(a,resBornMG/resBorn, c=color[len(types)-1], linewidth=0.5, where='post')
    axis[1].step(a,resMGExt[0], c=color[len(types)-1], linewidth=0.5, alpha=0.3,where='post')
    axis[1].step(a,resMGExt[1], c=color[len(types)-1], linewidth=0.5, alpha=0.3,where='post')
    axis[1].fill_between(a,resMGExt[0],resMGExt[1],alpha=0.5,facecolor=color[len(types)-1],linewidth=1,antialiased=True,step='post')

    ML, MR, resgg = loadgg("Expand",resgg)
    ML, MR, resqqb = loadqqb("Expand",resqqb)
    ...
    print("NLL_1/NLO = ",np.mean(combine[0]/resNLO))
    
    axis[0].step(ML,combine[0], c=color[-1], linewidth=0.5,where='post')
    axis[0].fill_between(ML,combineExt[0],combineExt[1],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[-1],label=r'${\rm \mathbf{'+labels[-1]+'}}$ ',step='post')
    ...
    DeltaComb=np.mean((combineExt[1]-combineExt[0])/combine[0])
    print("Delta Combination = ",DeltaComb,"Delta NLO = ",DeltaNLO)
    print("NLL_2/NLO = ",np.mean(combine[0]/resNLO))

    
    axis[0].step(ML,combine[0], c=color[-2], linewidth=0.5,where='post')
    axis[0].fill_between(ML,combineExt[0],combineExt[1],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[-2],label=r'${\rm \mathbf{'+labels[-2]+'}}$ ',step='post')
    axis[0].step(ML,combineExt[0], c=color[-2], linewidth=0.5, alpha=0.3,where='post')
    axis[0].step(ML,combineExt[1], c=color[-2], linewidth=0.5, alpha=0.3,where='post')

    axis[0].step(MR[-2:],combine[0][-2:], c=color[-2], linewidth=0.5,where='pre')
    axis[0].fill_between(MR[-2:],combineExt[0][-2:],combineExt[1][-2:],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[-2],step='pre')
    axis[0].step(MR[-2:],combineExt[0][-2:], c=color[-2], linewidth=0.5, alpha=0.3,where='pre')
    axis[0].step(MR[-2:],combineExt[1][-2:], c=color[-2], linewidth=0.5, alpha=0.3,where='pre')
    ...
    axis[1].step(ML,combine[0],label=r'${\rm \mathbf{'+labels[-2]+'}}$ ', c=color[-2], linewidth=0.5,where='post')
    axis[1].step(ML,combineExt[0], c=color[-2], linewidth=0.5, alpha=0.3,where='post')
    axis[1].step(ML,combineExt[1], c=color[-2], linewidth=0.5, alpha=0.3,where='post')
    axis[1].fill_between(ML,combineExt[0],combineExt[1],alpha=0.5,facecolor=color[-2],linewidth=1,antialiased=True,step='post')

    axis[1].step(MR[-2:],combine[0][-2:], c=color[-2], linewidth=0.5,where='pre')
    axis[1].step(MR[-2:],combineExt[0][-2:], c=color[-2], linewidth=0.5, alpha=0.3,where='pre')
    axis[1].step(MR[-2:],combineExt[1][-2:], c=color[-2], linewidth=0.5, alpha=0.3,where='pre')
    axis[1].fill_between(MR[-2:],combineExt[0][-2:],combineExt[1][-2:],alpha=0.5,facecolor=color[-2],linewidth=1,antialiased=True,step='pre')
    ...
    plt.savefig('ComparisonExpand.svg')
    plt.savefig('ComparisonExpand.png')
    plt.legend([])
    plt.close()
