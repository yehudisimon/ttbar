...
double H0(double *x, int m, int n, int chan, double M2)
{
  ...
  if(chan==0) dR=double(Nc);
  else if(chan==1) dR=pow(Nc,2)-1.;


  if(chan==0)
    {
      if(m*n==1) res=1.+pow(Xbet,2)+4.*Mt*Mt/M2;
    }
  ...
  return res*pow(alpha_s,2)*3./8./dR;
}
...
double H1(double *x, int m, int n, int chan, double M2)
{
  ...
  double Xbet=0, res=0, bet=0, t=0,dR=0.;
  ...
  res=2.*std::real(H[m][n])*pow(alpha_s,2)*3./8./dR;;
