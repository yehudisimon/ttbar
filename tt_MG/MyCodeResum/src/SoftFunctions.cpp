...
std::complex<double> GammGlobal(double *x, int m, int n, int chan, double M2)
{
  ...
  //if(std::isfinite(abs(res))==0) std::cout << " Gamma not finite ! " << std::endl;
  return res;  
}
  ...
  if(m==n)
    {
      if(chan==0)
	{
	  if(n==0) res= double(Nc);
	  else if(n==1) res=CF/2.;
	}
	...
	{
	  if(n==0) res= double(Nc);
	  else if(n==1) res=double(Nc)/2.;
	  else if(n==2) res=(pow(Nc,2)-4.)/2./double(Nc);
	}
    }
...
double S1(double *x, int m, int n, int chan, double M2)
{
  ...
  //if(std::isfinite(res)==0) std::cout << " S1 not finite ! " << std::endl;
