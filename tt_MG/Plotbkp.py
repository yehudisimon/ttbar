...
def BuildPlot():
    ...
    for i in range(len(labels)):
        if(i<2):
            ML, MR, res, resExt = loadataMycode(types[i],resS,resExtS)

            if(i==0) resNLOMG=np.copy(res)
            axis[0].step(ML,res, c=color[i], linewidth=0.5,where='post')
            axis[0].fill_between(ML,resExt[0],resExtS[1],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[i],label=r'${\rm \mathbf{'+labels[i]+'}}$ ',step='post')
            ...
            axis[0].step(MR[-2:],res[-2:], c=color[i-1], linewidth=0.5,where='pre')
    ...
    axis[1].step(MLS,Dummy,  label=r''+labels[i]+' ', c=color[i], linewidth=0.5,where='post')
    axis[1].step(MRS[-2:],Dummy[-2:], c=color[i], linewidth=0.5,where='pre')

    axis[1].step(MLS,resExtS[0], c=color[i], linewidth=0.5, alpha=0.3,where='post')
    axis[1].step(MLS,resExtS[1], c=color[i], linewidth=0.5, alpha=0.3,where='post')
    axis[1].fill_between(MLS,resExtS[0],resExtS[1],alpha=0.5,facecolor=color[i],linewidth=1,antialiased=True,step='post')

    axis[1].step(MRS[-2:],resExtS[0][-2:], c=color[i], linewidth=0.5, alpha=0.3,where='pre')
    axis[1].step(MRS[-2:],resExtS[1][-2:], c=color[i], linewidth=0.5, alpha=0.3,where='pre')
    axis[1].fill_between(MRS[-2:],resExtS[0][-2:],resExtS[1][-2:],alpha=0.5,facecolor=color[i],linewidth=1,antialiased=True,step='pre')

    
    i+=1
        
    ML, MR, res, resExt= loadataMG("NLO",res,resExt)
    resNLOMG=np.copy(res)
    ...
    axis[1].step(ML,res, c=color[i], linewidth=0.5,where='post')
    ...
    MLS, MRS, resS, resExtS = loadataMycode("Resum",resS,resExtS)
    
    axis[0].step(MLS,resS, c=color[i], linewidth=0.5,where='post')
    axis[0].fill_between(MLS,resExtS[0],resExtS[1],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[i],label=r'${\rm \mathbf{'+labels[i]+'}}$ ',step='post')
    axis[0].step(MLS,resExtS[0], c=color[i], linewidth=0.5, alpha=0.3,where='post')
    axis[0].step(MLS,resExtS[1], c=color[i], linewidth=0.5, alpha=0.3,where='post')

    axis[0].step(MRS[-2:],resS[-2:], c=color[i-1], linewidth=0.5,where='pre')
    axis[0].fill_between(MRS[-2:],resExtS[0][-2:],resExtS[1][-2:],alpha=0.5,linewidth=1,antialiased=True,facecolor=color[i],step='pre')
    axis[0].step(MRS[-2:],resExtS[0][-2:], c=color[i][-2:], linewidth=0.5, alpha=0.3,where='pre')
    axis[0].step(MRS[-2:],resExtS[1][-2:], c=color[i], linewidth=0.5, alpha=0.3,where='pre')
        
    for a in range(len(resExtS)):
        resExtS[a]/=resLOS
    resS/=resLOS

    axis[1].step(MLS,resS,  label=r''+labels[i]+' ', c=color[i], linewidth=0.5,where='post')
    axis[1].step(MRS[-2:],resS[-2:], c=color[i], linewidth=0.5,where='pre')
