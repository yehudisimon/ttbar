...
// -------- Functions --------------------------------- //
double H1(double*,int,int,int,double);
...
double S1(double*,int,int,int,double);
double S1t(double*,int,int,int,double);
  ...
  for(int k=0; k < Len; k++)
      ...
      for(int l=0; l < Len; l++)
	{
	  for(int mm=0; mm < Len; mm++)
		...
		{
		  Ht[k][l]+=R[k][mm]*(H0(x,mm,nn,chan,M2)+H1(x,mm,nn,chan,M2)*alpha_s/2./M_PI)*Rdag[nn][l];
		  //Ht[k][l]+=R[k][mm]*H0(x,mm,nn,chan,M2)*Rdag[nn][l]; //Without virtual
		  St[k][l]+=Rdagm1[k][mm]*(S0(mm,nn,chan)+S1t(x,mm,nn,chan,M2))*Rm[nn][l];
		}
	    }
	  DeltaProc=exp((Lamb[l]+std::conj(Lamb[k]))/2./beta0*log(1.-alpha_s/M_PI*beta0*log(Nb)));
	  St[k][l]*=DeltaProc;
	}
    }
   ...
   //std::cout << " Resummed res = " << res << std::endl;
   return res;
...
std::complex<double> TraceHSExpGlobal(double *x, int chan, double M2)
{
  ...
  std::complex<double> res=1., Nb;
  std::complex<double> **R, **Rm, **Rdag, **Rdagm1, *Lamb;
  //std::complex<double> Ht[Len][Len]={0}, St[Len][Len]={0}, i(0.0,1.0);
  ...
  // 	      for(int nn=0; nn < Len; nn++)
  // 		{
  // 		  Ht[k][l]+=R[k][mm]*H0(x,mm,nn,chan,M2)*Rdag[nn][l];	// Virtual contribution turned off
  // 		  St[k][l]+=Rdagm1[k][mm]*S0(mm,nn,chan)*Rm[nn][l];
  // 		}
  // 	    }
  // 	}
  //   }
  ...
  res=0.; // Compute the trace of Hard and Soft
   for(int ka=0; ka < Len; ka++)
       ...
       for(int ji=0; ji < Len; ji++)
	 {
	   ...
	   res+=S0(ka,ji,chan)*H1(x,ji,ka,chan,M2)*alpha_s/2./M_PI; //Virtual contribution
	   //res+=(S0(ka,ji,chan)+S1t(x,ka,ji,chan,M2))*H0(x,ji,ka,chan,M2);
	 }
     }
  ...
  if(chan==0)
    {
      ...
      //fAB=2.*Nf/(Npdf-1.)/(Npdf*Npdf)/(Npdf-1.);                                                         
    }
...
std::complex<double> GlobalGlobal(double *x, double sc, double M2) // Resummed formula, the numericall integration does the inversed Mellin transform
{
  ...
  res*=4.*M_PI/3.; // *8M_PI/3 from convention of H and /2 by flux factor (or dM -> dM^2)
  res*=bet/pow(M2,2);// Integration of M^2 dsigma/dM^2 * dM^2 * 1./M^2
...
double TotGlobal(double *x, double& sc, int& mel, double& M2)
{
  ...
  // std::cout << "Showing S1 quarks" << std::endl;
  // DisplayS1(x,0,M2);
  // std::cout << "Showing S1 gluons" << std::endl;
  // DisplayS1(x,1,M2);
  // std::cout << "Showing Gamma gluons" << std::endl;
  // DisplayG1(x,1,M2);
  // std::cout << "Showing Gamma quarks" << std::endl;
  // DisplayG1(x,0,M2);
  ...
  if(mel==2)
      ...
      for(int chan=0; chan < 2; chan++)
	{
	  ...
	  res+=std::imag((TraceHSExpGlobal(x,chan,M2)+ColinearExpGlobal(x,chan,M2)*temp)*MellinPDFGlobal(x,chan,M2)*GlobalGlobal(x,sc,M2));
	}
    }
      ...
      for(int chan=0; chan < 2; chan++)
	{
	  ...
	  res+=std::imag(temp*MellinPDFGlobal(x,chan,M2)*GlobalGlobal(x,sc,M2));
	}
    }
