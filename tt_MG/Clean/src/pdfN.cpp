         ...
         const gsl_multifit_nlinear_workspace *w)
{
  ...
  fprintf(stderr, "iter %2zu: A1 = %.4f, A2 = %.4f, A3 = %.4f, A4 = %.4f, A5 = %.4f, A6 = %.4f, A7 = %.4f, A8 = %.4f, cond(J) = %8.4f, |f(x)| = %.4f\n",
          iter,
          gsl_vector_get(x, 0),
          gsl_vector_get(x, 1),
          gsl_vector_get(x, 2),
...
void Fit(double A[8], double E[8], int flag, double xr, double weight_valence, double weight_sea, double weight_gluon) {
    ...
    //Carefull, only valid with CT18NLO !!
    PDF* F=mkPDF("CT18NLO",0);
    if (PDFID == 14400) {
      const PDF* F=mkPDF("CT18NLO",0);
    }
    else if(PDFID == 14000) {
      const PDF* F=mkPDF("CT18NNLO",0);
    }
    else if(PDFID == 21100) {
      const PDF* F=mkPDF("MSTW2008nlo68cl",0);
    }
    else {
      std::cout << "ERROR in PDF set" << std::endl;
      exit(0);
    }

    std::cout << "PDF chosen : " <<  F->memberID() << std::endl;
    ...
    std::cout << gsl_matrix_get(Jac,0,0) << std::endl;
