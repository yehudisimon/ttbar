...
def loadataHS(order,res,resExt):
    Mycode=np.loadtxt("Results/"+order+"dM.txt",delimiter=",")
...
def BuildPlot():
    ...
    for i in range(len(labels)):
        ML, MR, res, resExt = loadataHS(types[i],res,resExt)
        if(i==0) : resNLL=np.copy(res)
    ...
    plt.savefig('Implement.svg')
    plt.savefig('Implement.png')
    plt.legend([])
    plt.close()
